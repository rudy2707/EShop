...
-- Wed 16 Mar 2016 02:21:55 PM CET
...
CREATE TABLE IF NOT EXISTS `shop_test`.`tblCustomer` (
  ...
  `cusPassword` VARCHAR(200) NULL,
  PRIMARY KEY (`cusId`),
  UNIQUE INDEX `cusEmail_UNIQUE` (`cusEmail` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `shop_test`.`tblOrder` (
  ...
  CONSTRAINT `fk_cusId_ordCus`
    FOREIGN KEY (`ordCusId`)
    REFERENCES `shop_test`.`tblCustomer` (`cusId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_addrId_ordCus`
...
-- -----------------------------------------------------
-- Table `shop_test`.`tblCategory`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `shop_test`.`tblCategory` ;

CREATE TABLE IF NOT EXISTS `shop_test`.`tblCategory` (
  `catId` INT NOT NULL,
  `catLabel` VARCHAR(100) NULL,
  PRIMARY KEY (`catId`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `shop_test`.`tblProduct` (
  ...
  PRIMARY KEY (`prodId`),
  INDEX `fk_tblProduct_tblCategory1_idx` (`prodCategory` ASC),
  CONSTRAINT `fk_prodCategory_catId`
    FOREIGN KEY (`prodCategory`)
    REFERENCES `shop_test`.`tblCategory` (`catId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `shop_test`.`linkOrderProduct` (
  ...
  INDEX `ordId_idx` (`linkOrdId` ASC),
  CONSTRAINT `fk_ordId_ordPro`
    FOREIGN KEY (`linkOrdId`)
    REFERENCES `shop_test`.`tblOrder` (`ordId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_prodId_ordPro`
